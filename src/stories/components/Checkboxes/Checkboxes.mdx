...
import * as CheckboxStories from './Checkboxes.stories';
import { Hint, InsetText } from '../../../components';
import { Canvas, Story } from '@storybook/addon-docs'; import { GovScript } from '../../extras/GovScript';
...
If you’re asking just one question per page as recommended, you can set the contents of the `<Legend>` component as the page heading by setting the prop `isPageHeading` to `true`. This is good practice as it means that users of screen readers will only hear the contents once.
...
<Canvas of={CheckboxStories.conditional} />
